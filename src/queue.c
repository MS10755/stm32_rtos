...
Queue_t * queue_create(queueSize_t size,queueSize_t itemSize,Queue_type_t type){
    ...
    if(type == QUEUE_TYPE_NORMAL){
        newQueue->storage = (void*)((uint32_t)p_queue + sizeof(Queue_t));
...
static queueSize_t queue_convert_index(Queue_t * queue,void * p){
    return (uint32_t)((uint32_t)p - (uint32_t)queue->storage)/queue->itemSizeBytes;
}
...
static queueRet_t queue_move_write_index(Queue_t * queue){
    ...
    queue->p_write = (void*)((uint32_t)queue->storage + queue->itemSizeBytes * nextIndex);
    return QUEUE_OK;
}
...
static queueRet_t queue_move_read_index(Queue_t * queue){
    ...
    queue->isFull = false;
    queue->p_read = (void*)((uint32_t)queue->storage + queue->itemSizeBytes * nextIndex);
    return QUEUE_OK;
}
